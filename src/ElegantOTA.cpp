...
{
#if defined(ESP8266)
  this->setID(String((uint32_t)ESP.getChipId(), HEX).c_str(), "ESP8266");
#elif defined(ESP32)
  this->setID(String((uint32_t)ESP.getEfuseMac(), HEX).c_str());
#endif
}

void ElegantOtaClass::setID(const char *id, const char *hw)
{
#if defined(ESP8266)
  snprintf(_id, sizeof(_id), "{ \"id\": \"%s\", \"hardware\": \"%s\" }", id, hw);
#elif defined(ESP32)
